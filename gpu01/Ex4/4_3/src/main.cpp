...
extern void bankConflictsRead_Wrapper(dim3 gridSize, dim3 blockSize, int shmSize, float* dummy, int stride, long long int* clocks);
...
int main ( int argc, char * argv[] )
{
	...
	long long int hClocks = 0;
	long long int*dClocks = NULL;
	cudaMalloc ( &dClocks, sizeof ( long long int ) );
	...
	if ( chCommandLineGetBool ( "shared2register_conflict", argc, argv ) ) {
		cudaError_t error = cudaMemcpy ( &hClocks, dClocks, sizeof ( long long int), cudaMemcpyDeviceToHost );
