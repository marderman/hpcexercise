...
// Kernel Wrappers
//
extern void globalMem2SharedMem_Wrapper(dim3 gridSize, dim3 blockSize, int shmSize, float* d_mem, float* dmem_B, int elements);
extern void SharedMem2globalMem_Wrapper(dim3 gridSize, dim3 blockSize, int shmSize, float* d_mem, float* dmem_B, int elements);
...
main ( int argc, char * argv[] )
{
	...
	float* d_memoryB = NULL;
	cudaMalloc ( &d_memoryA, static_cast <size_t> ( optMemorySize ) ); // optMemorySize is in bytes
	cudaMalloc ( &d_memoryB, static_cast <size_t> ( optMemorySize ) ); // optMemorySize is in bytes
	...
	float h_memoryA[optMemorySize/sizeof(float)];
	// h_memoryA = malloc(optMemorySize);
	for(int i = 0; i < optMemorySize/sizeof(float); i++)
	{
		h_memoryA[i] = i;
	}
	cudaError_t error = cudaMemcpy ( d_memoryA, h_memoryA, optMemorySize, cudaMemcpyHostToDevice);
	...
	for ( int i = 0; i < optNumIterations; i++ )
	{
		...
		{
			globalMem2SharedMem_Wrapper( grid_dim, block_dim, optMemorySize, d_memoryA, d_memoryB, optMemorySize/sizeof(float));
			// cudaMemcpy ( h_memoryA, d_memoryB, optMemorySize, cudaMemcpyDeviceToHost);
		}
		else if ( chCommandLineGetBool ( "shared2global", argc, argv ) )
		{
			SharedMem2globalMem_Wrapper( grid_dim, block_dim, optMemorySize, d_memoryA, d_memoryB, optMemorySize/sizeof(float));
		}
	...
	if ( chCommandLineGetBool ( "global2shared", argc, argv ) ) {
		std::cout << "Copy global->shared;size;" << optMemorySize << ";gDim;" << grid_dim.x << ";bDim;" << block_dim.x;
		//std::cout << ", time=" << kernelTimer.getTime(optNumIterations) << 
		std::cout.precision ( 2 );
		std::cout << ";bw;" << std::fixed << ( optMemorySize * grid_dim.x ) / kernelTimer.getTime(optNumIterations) / (1E09) << ";GB/s" << std::endl;
	}
	
	if ( chCommandLineGetBool ( "shared2global", argc, argv ) ) {
		std::cout << "Copy shared->global;size;" << optMemorySize << ";gDim;" << grid_dim.x << ";bDim;" << block_dim.x;
		//std::cout << ", time=" << kernelTimer.getTime(optNumIterations) << 
		std::cout.precision ( 2 );
		std::cout << ";bw;" << std::fixed << ( optMemorySize * grid_dim.x ) / kernelTimer.getTime(optNumIterations) / (1E09) << ";GB/s" << std::endl;
	}
