...
extern void globalMemCoalescedKernel_Wrapper(dim3 gridDim, dim3 blockDim, int* dst, int* src, int elements);
extern void globalMemStrideKernel_Wrapper(dim3 gridDim, dim3 blockDim, int* dst, int* src,int stride /*TODO Parameters*/);
...
main ( int argc, char * argv[] )
{
    ...
    cudaError_t code1 = cudaMalloc((void**)&d_memoryB, static_cast <size_t> ( optMemorySize ));
cudaMalloc((void**)&d_memoryA, static_cast <size_t> ( optMemorySize ));
    cudaError_t code2 = cudaMalloc((void**)&d_memoryB, static_cast <size_t> ( optMemorySize ));

    if ( !h_memoryA || !h_memoryB || !d_memoryA || !d_memoryB ) {
        std::cout << "\033[31m***" << std::endl
                  << "*** Error - Memory allocation failed" << std::endl
                  << "***\033[0m" << std::endl;
        std::cout << "GPUassert: " << code1 << std::endl;
        std::cout << "GPUassert: " << code2 << std::endl;
        std::cout << "h_memoryA " << h_memoryA << std::endl;
        std::cout << "h_memoryB " << h_memoryB << std::endl;
        std::cout << "d_memoryA " << d_memoryA << std::endl;
        std::cout << "d_memoryB " << d_memoryB << std::endl;
        std::cout << "optMemorySize " << optMemorySize << std::endl;
    ...
    for ( int i = 0; i < optMemCpyIterations; i ++ ) {
        // H2D copy
        cudaMemcpy(d_memoryA, h_memoryB, optMemorySize, cudaMemcpyHostToDevice);
    ...
    for ( int i = 0; i < optMemCpyIterations; i ++ ) {
        // D2H copy
        cudaMemcpy(h_memoryA, d_memoryA, optMemorySize, cudaMemcpyHostToHost);
        cudaDeviceSynchronize();
    }
    memCpyD2HTimer.stop ();
    ...
    for ( int i = 0; i < optNumIterations; i++ ) {
        ...
        if ( chCommandLineGetBool( "global-coalesced", argc, argv ) ) {
            ...
            globalMemStrideKernel_Wrapper(grid_dim, block_dim, d_memoryA, d_memoryB, optStride /*TODO Parameters*/);
        } else if ( chCommandLineGetBool( "global-offset", argc, argv ) ) {
    ...
    if ( chCommandLineGetBool ( "global-stride", argc, argv ) ) {
        std::cout << "Strided;" << optStride << ";copy of global memory, size;" << optMemorySize << ";gDim;" << grid_dim.x << ";bDim;"<< block_dim.x;
        //std::cout << ", time=" << kernelTimer.getTime(optNumIterations) << 
        std::cout.precision ( 2 );
        std::cout << ";bw;" << std::fixed <<( grid_dim.x * block_dim.x ) * sizeof (int) / kernelTimer.getTime (optNumIterations ) / ( 1E09 ) << ";GB/s" << std::endl;
    }
